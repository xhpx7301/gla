...
已安装采集器时，运行 `alloy`，选择 `11. 配置采集模块 -> 1. 主机指标`，输入 VictoriaMetrics Remote Write 地址和认证用户名。重新部署时脚本会安全询问指标接口密码。
...
文件权限为 `600`。安装后运行 `alloy` 打开采集端管理菜单。菜单顶部会显示 Alloy 容器、Xray 日志、安全日志、主机指标、GeoIP、SSH/UFW 聚合流量和 3x-ui 流量采集的当前状态。选择菜单 `11. 配置采集模块` 可以分别启用、更新或关闭这些能力，不需要手写环境变量。菜单 `6` 可分别查看 Alloy 与 3x-ui API 导出器日志；导出器容器处于“运行中”只代表进程已启动，是否成功访问面板 API 应以导出器日志和 Grafana 指标为准。
...
5. 再次运行 `alloy`，选择 `11. 配置采集模块 -> 1. 主机指标`，填写指标写入地址；安装器会询问 Loki 和指标接口密码。
6. 采集服务器同时运行 3x-ui 时，选择 `11. 配置采集模块 -> 5. 3x-ui API 流量`。它与主机指标共享 VictoriaMetrics 地址，但可以独立开关。
